...
export default function SeaquestGame({ imageFormat = 'png' }: SeaquestGameProps) {
        ...
        if (window.CrazyGames && window.CrazyGames.SDK && window.CrazyGames.SDK.ad) {
            window.CrazyGames.SDK.ad.requestAd('rewarded', {
                adFinished: () => {
                    ...
                    g.gameOver = false;
                    g.playerLives = 1; // Give 1 life
                    g.oxygen = MAX_OXYGEN;
                    if (canvasRef.current) {
                        g.diver.x = canvasRef.current.width / 2;
                        g.diver.y = canvasRef.current.height / 2;
                    }
                    g.diver.isShieldActive = true;
                    g.diver.shieldTimer = 180; // 3 seconds immunity

                    // Resume game
                    setUiGameState("playing");
                    g.lastUpdateTime = performance.now();

                    // Notify gameplay started again
            ...
            console.log("CrazyGames SDK not available for rewarded ad");
            // For testing without SDK, just revive
            if (!gameInstance.current) return;
            const g = gameInstance.current;
            g.gameOver = false;
            g.playerLives = 1;
    ...
    useEffect(() => {
        ...
        if (window.CrazyGames && window.CrazyGames.SDK && window.CrazyGames.SDK.data) {
            try {
                const savedScore = window.CrazyGames.SDK.data.getItem('seaquest_highscore');
                if (savedScore) {
            ...
            try {
                const savedScore = localStorage.getItem('seaquest_highscore');
                if (savedScore) {
                    setHighScore(parseInt(savedScore, 10));
                }
            } catch (e) {
        ...
        const endGame = () => {
            ...
            if (window.CrazyGames && window.CrazyGames.SDK) {
                ...
                if (game.score > highScore) {
                    ...
                    try {
                        if (window.CrazyGames.SDK.data) {
                            window.CrazyGames.SDK.data.setItem('seaquest_highscore', game.score.toString());
                        }
                    } catch (e) {
                ...
                // Request an interstitial ad on game over
                if (window.CrazyGames.SDK.ad) {
                    window.CrazyGames.SDK.ad.requestAd('midgame');
                }
            } else {
                // Local high score fallback
                if (game.score > highScore) {
                    setHighScore(game.score);
        ...
        <div className="canvas-container">
            ...
            {uiGameState === "start" && (
                <div className="game-overlay" style={{ background: 'transparent' }}>
                    ...
                    {/* Content */}
                    <div style={{ opacity: 0.7, marginBottom: '20px', position: 'relative', zIndex: 1 }}>
                    ...
                    <button onClick={handleStartGame} style={{ position: 'relative', zIndex: 1 }}>START GAME</button>
                </div>
            )}
            ...
            {uiGameState === "over" && (
                <div className="game-overlay">
                    ...
                    <p className="score-display">High Score: {highScore}</p>
                    <p className="score-display">Enemies Defeated: {finalKillCount}</p>
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'center' }}>
                        <button onClick={handleRestart}>PLAY AGAIN</button>
                        <button onClick={handleRevive} style={{ backgroundColor: '#ffaa00' }}>REVIVE (AD)</button>
                    </div>
                </div>
            )}
            ...
            {uiGameState === "playing" && (
                <TouchControls onControlChange={(control, active) => {
                    if (!gameInstance.current) return;
                    const key = control === 'Space' ? 'Space' : control;
                    // Handle space key specially as it might be mapped differently
                    if (key === 'Space') {
                        gameInstance.current.keys[' '] = active;
                        gameInstance.current.keys['Space'] = active;
                    } else {
                        gameInstance.current.keys[key] = active;
                    }
                }} />
            )}
        </div>
